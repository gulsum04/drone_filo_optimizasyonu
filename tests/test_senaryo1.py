...
#DİNAMİK VERİ : 5 drone, 20 teslimat, 2 no-fly zone TÜM ALGORİTMALAR
# Veri üretimi fonksiyonları

def generate_random_drones(count=5):
    drones = []
...
def generate_random_deliveries(count=20):
    deliveries = []
    for i in range(count):
        delivery = Teslimat(
            id=i,
...
def generate_random_noflyzones(count=2):
    nfzs = []
    for i in range(count):
...
def test_senaryo1():
    print("\n🚀 Senaryo 1: Dinamik 5 drone, 20 teslimat, 2 no-fly zone ile A*, CSP+A*, GA")
    start = time.time()
