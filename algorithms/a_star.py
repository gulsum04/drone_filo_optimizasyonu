...
def a_star(start, goal, weight, priority, nfz_list, current_time=0):
    """
    A* algoritması ile drone'un başlangıç konumundan hedef teslimat noktasına 
    en uygun rotayı bulur.
    """
    open_set = []
    ...
    iteration = 0
    MAX_ITER = 50000  # Artırıldı (önceki: 10000)
    ...
    return None, float('inf')  # Ulaşılmazsa

def generate_neighbors(pos):
    """
    Komşu noktaları oluşturur. 8 yönlü sabit adımda ilerleme yapar.
    """
    step = 5
    return [
        (pos[0] + step, pos[1]),
...
def in_noflyzone(pos, nfz_list, current_time=0):
    """
    Belirli bir pozisyonun, zaman penceresi dahilinde no-fly zone içerisinde olup olmadığını kontrol eder.
    """
    for nfz in nfz_list:
...
def reconstruct_path(came_from, current):
    """
    Ulaşılan hedefe giden yolu geri sararak oluşturur.
    """
    total_path = [current]
